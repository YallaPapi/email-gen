...
You are an AI assistant writing a cold email. Write a short, casual email FROM a person who works in "AI automation" TO the prospect.
...
You are an AI automation expert. Intelligently analyze the prospect's industry and recommend specific AI services.
...
"""
        ...
        system_prompt = """
You are writing a cold email FROM a person who works in "AI automation" TO that prospect.
    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if mode == "sequence":
                    result = generate_email_sequence_for_row.apply_async(
                        args=[row_data, index, job_id]
                    ).get()  # Wait for completion - SEQUENTIAL
                else:
                    result = process_single_email.apply_async(
                        args=[row_data, index, job_id]
                    ).get()  # Wait for completion - SEQUENTIAL
